...
async function getPlayerModule() {
  ...
  const module = await import("./features/player.js?v=21");
  module.initPlayer({
    appState,
    DOM,
    ErrorHandler,
    ...
    new MutationObserver(async (mutations) => {
      for (const m of mutations) {
        if (
          ...
          cinemaEl.classList.contains("show")
        ) {
...
async function refreshDataInBackground() {
  try {
    ...
    console.warn("No se pudo actualizar background", e);
  }
}
...
function setupNavigation() {
  const navContainers = document.querySelectorAll(
    ".main-nav ul, .mobile-nav ul, .bottom-nav, #profile-hub-container, .header-right",
  );
...
async function switchView(filter) {
  ...
  if (
    ...
    if (container) {
      ...
      if (filter === "settings")
        getProfileModule().then((m) => m.renderSettings());
    }
    return;
  }
  ...
  document.querySelectorAll(".close-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      if (btn.closest("#review-form-modal")) {
        ...
        const cinemaOpen = document
          .getElementById("cinema")
          ?.classList.contains("show");
        const seriesOpen = document
          .getElementById("series-player-modal")
          ?.classList.contains("show");
        if (!cinemaOpen && !seriesOpen)
...
function changeHeroMovie(itemObj) {
  ...
  setTimeout(() => {
    ...
    const imageUrl =
      appState.hero.preloadedImages.get(cacheKey) ||
    ...
    const heroTitleContainer = heroContent.querySelector(
    ...
    if (data.logoUrl) {
      ...
      const heroLogoContainer = heroTitleContainer.querySelector(
        ".hero-logo-container",
      );
...
function generateContinueWatchingCarousel(snapshot) {
  ...
  const seriesOnly = historyItems.filter(
    (item) => item.type === "series" && !isSpecialSeason(item.season),
  );
...
async function openDetailsModal(id, type, triggerElement = null) {
  try {
    ...
    if (data.logoUrl) {
      ...
      const logoContainer = detailsTitleEl.querySelector(
    ...
    if (detailsMeta) {
      ...
      if (modalRating) {
        ...
        if (reviewsModule && reviewsModule.getStarsHTML) {
          ratingBadge.innerHTML = reviewsModule.getStarsHTML(
            modalRating,
            false,
          );
        } else {
        ...
        const reviewCount =
          appState.content.reviewCounts && appState.content.reviewCounts[id];
        if (reviewCount) {
      ...
      const isProximamente =
        !isSeries &&
        data.estado &&
        data.estado.toLowerCase() !== "vetada" &&
      ...
      const originalTitle =
        rawAltTitle && normStr(rawAltTitle) !== normStr(data.title || "")
          ? rawAltTitle
          : null;
    ...
    const _bannerUrl =
      data.banner && data.banner.length > 5 ? data.banner : null;
    if (panel) {
    ...
    if (detailsButtons) {
      ...
      const getProximamenteLabel = (estado) => {
        ...
        if (lower === "proximamente" || lower === "próximamente")
          return "Próximamente";
      ...
      const isMantenimiento =
        ...
        data.estado &&
        data.estado.toLowerCase() === "mantenimiento";
      const proximamenteLabel =
        !isSeries && data.estado ? getProximamenteLabel(data.estado) : null;
      ...
      if (isVetada) {
        ...
        if (isSeries) {
          ...
          const randomVal = String(data.randomValue || data.random || "")
            .trim()
            .toLowerCase();
          const isRandomEnabled = ["si", "sí", "yes", "true", "1"].includes(
            randomVal,
          );
          ...
          if (isRandomEnabled) {
            ...
            randomBtn.onclick = async () => {
              ...
              const randomIndex = Math.floor(
                Math.random() * allEpisodes.length,
              );
      ...
      if (!isSeries && auth.currentUser) {
        ...
        eyeBtn.title = isWatched
          ? "Quitar de vistas (volverá a la ruleta)"
          : "Marcar como vista (no saldrá en la ruleta)";
...
function setupAuthListeners() {
  ...
  if (DOM.loginBtnHeader)
    DOM.loginBtnHeader.onclick = (e) => {
      e.preventDefault();
      openAuthModal(true);
    };
  if (DOM.registerBtnHeader)
    DOM.registerBtnHeader.onclick = (e) => {
      e.preventDefault();
      openAuthModal(false);
    };
...
function updateUIAfterAuthStateChange(user) {
  ...
  if (user) {
    ...
    if (DOM.userGreetingBtn)
      DOM.userGreetingBtn.textContent = `Hola, ${userName}`;
    ...
    db.ref(`users/${user.uid}/watchlist`).once("value", (snapshot) => {
      appState.user.watchlist = snapshot.exists()
...
function addToHistoryIfLoggedIn(contentId, type, episodeInfo = {}) {
  ...
  const historyTitle = isSeries
    ? totalSeasonsForTitle > 1
      ? `${itemData.title}: T${episodeInfo.season}`
      : itemData.title
    : itemData.title;
...
async function addToWatchlist(contentId) {
  ...
  await ErrorHandler.firebaseOperation(async () => {
    ...
    document
      .querySelectorAll(`.btn-watchlist[data-content-id="${contentId}"]`)
      .forEach((button) => {
...
function appendMyListBatch() {
  ...
  const nextBatch = myListDataCache.slice(
    myListRenderedCount,
    myListRenderedCount + BATCH_SIZE,
  );
  ...
  if (myListRenderedCount < myListDataCache.length) {
    if (!loadBtn) {
      ...
      loadBtn.style.cssText =
        "display: block; margin: 30px auto; min-width: 200px;";
...
function appendHistoryBatch() {
  ...
  const nextBatch = historyDataCache.slice(
    historyRenderedCount,
    historyRenderedCount + BATCH_SIZE,
  );
...
function createMovieCardElement(
  ...
  card.onclick = (e) => {
    ...
    if (seasonMatch) {
      ...
      (async () => {
        const player = await getPlayerModule();
        player.openPlayerToEpisode(id, options.season, options.lastEpisode);
      })();
    } else if (
...
function setupRatingsListener() {
  console.log(
    "ℹ️ setupRatingsListener: Ya configurado en el módulo de reviews",
  );
}
