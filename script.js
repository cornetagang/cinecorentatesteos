...
async function getPlayerModule() {
  ...
  const module = await import("./features/player.js?v=26");
  module.initPlayer({
    appState,
    DOM,
    ErrorHandler,
...
function _bentoDailyPick(movies, series, watchedIds = new Set(), topGenres = []) {
  ...
  // Sin historial del usuario: pick aleatorio clásico con semilla diaria
  if (!topGenres.length) return pool[seed % pool.length];

  // Con historial: priorizar contenido no visto con géneros favoritos
  const unwatched = pool.filter(({ id }) => !watchedIds.has(id));
  // Fallback: si el usuario vio casi todo, usar el pool completo
  const candidates = unwatched.length >= 3 ? unwatched : pool;

  // Puntuar cada item según cuántos géneros top coincide
...
function _bentoPopulateMain(id, data, type) {
  ...
  playBtn.onclick = async () => {
    ...
    if (type === "series") {
      player.playSeriesInDetailView(id);
    } else {
...
function _populateMobileHero(id, data, type) {
  ...
  if (playBtn) {
    playBtn.onclick = async () => {
      const player = await getPlayerModule();
      if (type === "series") openSeriesDetailView(id);
      else player.openPlayerModal(id, (data.title || "").replace(/'/g, "\\'"));
    };
  }
...
function setupHero() {
  ...
  // Panel principal: pick random inmediato (no bloquea el render)
  const pick = _bentoDailyPick(movies, series);
  if (pick) _bentoPopulateMain(pick.id, pick.data, pick.type);

  // Si hay usuario logueado, refinar con sus géneros favoritos del historial
  const user = auth.currentUser;
  if (user) {
    db.ref(`users/${user.uid}/history`)
      .once("value")
      .then((snap) => {
        ...
        if (personalPick && personalPick.id !== (pick && pick.id)) {
          _bentoPopulateMain(personalPick.id, personalPick.data, personalPick.type);
        }
      })
      .catch(() => {}); // si falla el historial, queda el pick random
  }
...
// ── DEBUG: ciclar recomendación ───────────────────────────────
let _bentoDebugIndex = null;

window.bentoDebugNext = function () {
  const movies = appState.content.movies || {};
  const series = appState.content.series || {};
  const pool = [
    ...Object.entries(movies)
  ...
  if (_bentoDebugIndex === null) {
    // Arranca desde el pick del día actual
    const hoy = new Date();
    const seed =
      hoy.getFullYear() * 10000 + (hoy.getMonth() + 1) * 100 + hoy.getDate();
    _bentoDebugIndex = ((seed % pool.length) + 1) % pool.length;
  } else {
    _bentoDebugIndex = (_bentoDebugIndex + 1) % pool.length;
  }

  const pick = pool[_bentoDebugIndex];
  _bentoPopulateMain(pick.id, pick.data, pick.type);
  console.log(
    `🎬 Bento debug [${_bentoDebugIndex}/${pool.length - 1}]: ${pick.data.title} (${pick.type})`,
  );
};
...
async function openSeriesDetailView(id) {
  try {
    ...
    if (btnGroup) {
      ...
      console.log("[random-btn] data.random =", JSON.stringify(data.random), "| id =", id);
